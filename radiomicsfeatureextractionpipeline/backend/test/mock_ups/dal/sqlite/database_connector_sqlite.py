...
from test.mock_ups.dal.database_connector import DatabaseConnectorMockUp


class DatabaseConnectorSqliteMockUp(DatabaseConnectorSqlite, DatabaseConnectorMockUp):
    ...
    def __init__(self, connection_sting: str) -> None:
        ...
        self.execute_query_result_value: Optional[pd.DataFrame] = None

        self.close_connection_called_with_parameters: List[Dict[Optional[str], Any]] = []
        self.close_connection_return_value: Any = None
    ...
    def set_execute_non_query_return_value(self, return_value: Any) -> None:
        self.execute_non_query_return_value = return_value

    def get_execute_query_called_with_parameter(self):
        return self.execute_query_called_with_parameters

    def set_execute_query_result_value(self, result_value: pd.DataFrame) -> None:
        self.execute_query_result_value: pd.DataFrame = result_value

    def get_close_connection_called_with_parameters(self) -> List[Dict[Optional[str]]]:
        return self.execute_non_query_return_value

    def set_close_connection_return_value(self, return_value: Any) -> None:
